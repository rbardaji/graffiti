...
from .utils.db_manager import get_pid, add_values_v2


api = Namespace('hash', description='Hash management')


doy_payload = api.model('hash_content', {
...
@api.response(201, 'Hash file created')
@api.response(204, 'Email not found')
@api.response(401, 'Provide a valid Token')
@api.response(500, 'Server error')
class GetPostHash(Resource):
    """ Hash operations"""
    # @user_ns.marshal_list_with(authorization_model)
    ...
    def post(self, email):
        ...
        Post a hash file
        """
        # return make_files(pid, userId, version)
        return add_values_v2(api.payload, email, DOI=False)
