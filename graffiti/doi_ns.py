...
from .utils.db_manager import get_doi, post_doi, put_doi
...
class GetPostDOI(Resource):
    ...
    # @decorator_api.token_required
    @api.expect(number_payload)
    @save_request
    def put(self, user):
        """
        Update number of available DOIs per user.
        """
        return put_doi(user, api.payload['num'])
